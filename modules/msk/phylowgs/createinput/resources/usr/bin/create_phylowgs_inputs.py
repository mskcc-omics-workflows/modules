...
class CnvFormatter(object):
        ...
        for chrom, chrom_cnvs in cnvs.items():
            for cnv in chrom_cnvs:
                ...
                yield {
                    ...
                    "overlapping_variants": self._format_overlapping_variants(
                        overlapping_variants, cnv["major_cn"], cnv["minor_cn"]
                    ),
                }
    ...
    def format_and_merge_cnvs(self, cnvs, variants, cellularity):
        ...
        for current in formatted:
            last = merged[-1]
            assert np.all(current["cellular_prevalence"] <= cellularity) and np.all(
...
class Segmenter(object):
        ...
        for chrom, cnvs in cnv_set.items():
            ...
            start_pos = [
                (
                    c["start"],
                    "start",
                    (c["sample"], c["cell_prev"], c["major_cn"], c["minor_cn"]),
                )
                for c in cnvs
            ]
            end_pos = [
            ...
            while idx < len(positions):
                ...
                for (
                    pos,
                    pt_type,
                    (sampidx, cell_prev, major_cn, minor_cn),
                ) in points_at_locus:
                    if pt_type == "start":
                        log(
                            "Adding ",
                            (pos, pt_type, sampidx, cell_prev, major_cn, minor_cn),
                        )
    ...
    def _merge_adjacent(self, cncalls, allowed_gap=0):
        ...
        while idx < len(cncalls):
            ...
            while (
                idx < len(cncalls)
                and cncalls[idx]["chrom"] == adjacent[-1]["chrom"]
                and cncalls[idx]["major_cn"] == adjacent[-1]["major_cn"]
                and cncalls[idx]["minor_cn"] == adjacent[-1]["minor_cn"]
                and 0 <= cncalls[idx]["start"] - adjacent[-1]["end"] <= allowed_gap
            ):
...
class VariantAndCnvGroup(object):
    ...
    def _print_variant_differences(self, before, after, before_label, after_label):
        ...
        def _print_region(var):
            ...
            if containing_cnv is not None:
                log(
                    "%s\t[in %s-CN region chr%s(%s, %s)]"
                    % (
                        var_name,
                        region_type,
                        var.CHROM,
                        containing_cnv["start"],
                        containing_cnv["end"],
                    )
                )
            else:
    ...
    def format_variants(
        ...
        subsampled_formatted = list(
            formatter.format_variants(
                subsampled_variants,
                subsampled_ref_counts,
                subsampled_total_counts,
                error_rate,
                sex,
            )
        )
        nonsubsampled_formatted = list(
            formatter.format_variants(
                nonsubsampled_variants,
                nonsubsampled_ref_counts,
                nonsubsampled_total_counts,
                error_rate,
                sex,
            )
        )
...
def impute_missing_total_reads(total_reads, missing_variant_confidence):
    ...
    normalized_to_sample = np.dot(masked_total_reads, np.diag(1.0 / sample_means))
...
def extract_sample_data(
    ...
    assert len(vcf_files_and_samples) == len(
        vcf_types_and_samples
    ), "Must specify same number of VCF files and VCF types"
    srcs_and_dsts = [
        (vcf_files_and_samples, vcf_files),
        (vcf_types_and_samples, vcf_types),
    ]
...
def main():
    ...
    parser.add_argument(
        ...
        action="append",
        required=True,
        help="Type of VCF file for each sample, specified as <sample>=<vcf_type>. Valid VCF types are %s."
        % ",".join(all_vcf_types),
    )
    parser.add_argument(
        "-e",
    ...
    parser.add_argument(
        ...
        dest="missing_variant_confidence",
        type=restricted_float,
        default=1.0,
    ...
    parser.add_argument(
        "--regions",
        dest="regions",
        choices=("normal_cn", "normal_and_abnormal_cn", "all"),
        default="normal_and_abnormal_cn",
    ...
    parser.add_argument(
        ...
        dest="output_nonsubsampled_variants",
    ...
    parser.add_argument(
        ...
        dest="output_nonsubsampled_variants_cnvs",
    ...
    parser.add_argument(
        "vcf_files",
        nargs="+",
        help="Path to VCF file for each sample. Specified as <sample>=<VCF path>.",
    )
    ...
    num_samples = len(samples)
    variant_ids, ref_read_counts, total_read_counts = parse_variants(
        samples,
        vcf_files,
        vcf_types,
        args.tumor_sample,
        args.missing_variant_confidence,
    )
    ...
    subsampled_vars, nonsubsampled_vars = grouper.format_variants(
        args.sample_size, args.error_rate, priority_ssms, args.only_priority, sex
    )
